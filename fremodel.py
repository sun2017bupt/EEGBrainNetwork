...
class FreMyMixture(nn.Module):
    ...
    def forward(self, ng1_data, ng2_data, ng3_data, ng4_data, g_data, graph,step):
        n = ng1_data.size(0)
        x = torch.cat([ng1_data,ng2_data,ng3_data,ng4_data],dim=-1)
        ...
        tg_out = self.tg(g_data,graph,step)
        ...
        tg_out = self.fc(tg_out)
        tg_out = F.gelu(self.fc_e(tg_out))
        tg_out = self.fc_a(tg_out)
